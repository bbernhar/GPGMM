...
namespace gpgmm::d3d12 {
    ...
    Heap::Heap(Microsoft::WRL::ComPtr<ID3D12Pageable> pageable, uint64_t size, uint64_t alignment)
        : MemoryBase(size, alignment), mPageable(pageable) {
    }
    ...
    std::vector<Heap*>::const_iterator ResidencyList::begin() const {
        return mList.begin();
    }

    std::vector<Heap*>::const_iterator ResidencyList::end() const {
        return mList.end();
    }
