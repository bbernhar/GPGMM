...
namespace gpgmm { namespace d3d12 {
    ...
    std::unique_ptr<MemoryAllocation> BufferAllocator::AllocateMemory(uint64_t size,
                                                                      uint64_t alignment) {
        ASSERT(alignment == GetMemoryAlignment());
        ASSERT(size == mResourceSize);
        ...
        // pOptimizedClearValue is not supported for buffers.
        ComPtr<ResourceAllocation> resourceAllocation;
        if (FAILED(mResourceAllocator->CreateCommittedResource(
                mHeapType, D3D12_HEAP_FLAG_NONE, {size, alignment}, &resourceDescriptor,
                /*pOptimizedClearValue*/ nullptr, mInitialUsage, &resourceAllocation))) {
            return nullptr;
        }

        return std::make_unique<MemoryAllocation>(/*allocator*/ this, resourceAllocation->GetInfo(),
                                                  /*offset*/ 0, resourceAllocation->GetMemory());
    }
