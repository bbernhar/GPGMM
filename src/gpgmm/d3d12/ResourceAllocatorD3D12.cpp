...
namespace gpgmm::d3d12 {
    ...
    ResourceAllocator::ResourceAllocator(const ALLOCATOR_DESC& descriptor,
        ...
        ResidencyManager* residencyManagerObj =
            static_cast<ResidencyManager*>(mResidencyManager.Get());

        const bool isUMA =
            (IsResidencyEnabled()) ? residencyManagerObj->IsUMA() : mCaps->IsAdapterUMA();
    ...
    HRESULT ResourceAllocator::CreateResource(const ALLOCATION_DESC& allocationDescriptor,
        ...
        // behind a macro.
        if (mDebugAllocator) {
            ResourceAllocation* allocationObj = static_cast<ResourceAllocation*>(allocation.Get());
            ReturnIfFailed(allocationObj->SetDebugName(allocationDescriptor.DebugName));
            mDebugAllocator->AddLiveAllocation(allocationObj);
        }
        ...
        CheckAndReportAllocationMisalignment(*allocationObj);
#endif
    ...
    HRESULT ResourceAllocator::CreateResourceInternal(
        ...
        if (IsResidencyEnabled()) {
            ...
            heapProperties.MemoryPoolPreference =
                GetMemoryPool(heapProperties, residencyManagerObj->IsUMA());

            const DXGI_MEMORY_SEGMENT_GROUP segment = GetMemorySegmentGroup(
                heapProperties.MemoryPoolPreference, residencyManagerObj->IsUMA());

            DXGI_QUERY_VIDEO_MEMORY_INFO* currentVideoInfo =
                residencyManagerObj->GetVideoMemoryInfo(segment);
