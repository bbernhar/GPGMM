...
namespace gpgmm::d3d12 {
    ...
    std::unique_ptr<MemoryAllocation> ResourceHeapAllocator::TryAllocateMemory(
        ...
        ResidencyManager* residencyManagerObj = static_cast<ResidencyManager*>(mResidencyManager);
        if (residencyManagerObj != nullptr) {
            resourceHeapDesc.MemorySegmentGroup = GetMemorySegmentGroup(
                mHeapProperties.MemoryPoolPreference, residencyManagerObj->IsUMA());
        }

        IHeap* resourceHeap = nullptr;
        if (FAILED(Heap::CreateHeap(
                resourceHeapDesc, residencyManagerObj,
