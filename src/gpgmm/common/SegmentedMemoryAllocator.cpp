...
namespace gpgmm {
    ...
    std::unique_ptr<MemoryAllocation> SegmentedMemoryAllocator::TryAllocateMemory(
        ...
        IMemoryBase* memory = allocation.GetMemory();
        ASSERT(memory != nullptr);

        MemoryBase* memoryObj = reinterpret_cast<MemoryBase*>(memory);
        memoryObj->SetPool(segment);
    ...
    void SegmentedMemoryAllocator::DeallocateMemory(std::unique_ptr<MemoryAllocation> allocation) {
        ...
        MemoryBase* memoryObj = reinterpret_cast<MemoryBase*>(memory);
        MemoryPool* pool = memoryObj->GetPool();
        ASSERT(pool != nullptr);
