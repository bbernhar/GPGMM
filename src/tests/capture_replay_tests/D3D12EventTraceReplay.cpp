...
class D3D12EventTraceReplay : public D3D12TestBase, public CaptureReplayTestWithParams {
  protected:
    ...
    void RunTest(const TraceFile& traceFile,
            ...
            if (event["name"].asString() == "ResidencyManager.ExecuteCommandLists") {
                switch (*event["ph"].asCString()) {
                    case TRACE_EVENT_PHASE_INSTANT: {
                        ...
                        ResidencySet set = {};
                        const Json::Value& heaps = args["Heaps"];
                        for (auto heap : heaps) {
                            std::string heapId = heap["id_ref"].asString();
                            ASSERT_TRUE(createdHeapToID.find(heapId) != createdHeapToID.end());
                            set.Insert(createdHeapToID[heapId]);
                        }

                        ResidencySet* residencySets[] = {&set};
                        ASSERT_SUCCEEDED(residencyManager->ExecuteCommandLists(nullptr, nullptr,
                                                                               residencySets, 1));
        ...
        EXPECT_TRUE(createdResidencyManagerToID.empty());
